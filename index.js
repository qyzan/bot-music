...
async function playStaticSong(guild, song, message) {
    ...
    try {
        const stream = ytdl(song, { filter: 'audioonly', quality: 'highestaudio', highWaterMark: 1 << 25, liveBuffer: 4000 });
        ...
        message.channel.send(`🎶 Sekarang Memutar: **${videoInfo.videoDetails.title}**`);
...
async function playMixSong(guild, message) {
    ...
    try {
        const stream = ytdl(songUrl, { filter: 'audioonly', quality: 'highestaudio', highWaterMark: 1 << 25, liveBuffer: 4000 });
        ...
        message.channel.send(`🎶 Sekarang AutoPlay: **${videoInfo.videoDetails.title}**`);
        ...
        stream.on('error', err => {
            console.error(`⚠️ Stream error (Mix Mode): ${err.message}`);
            if (!serverQueue.loop) serverQueue.mixIndex++;
            playMixSong(guild, message);
        });
